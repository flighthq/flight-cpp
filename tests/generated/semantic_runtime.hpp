...
inline double update(flight::Map<flight::String, double> labels, flight::Set<flight::String> seen) {
  ...
  return (seen.has(flight::String("size")) ? ([&]() -> double { auto nullish_coalesce_left = labels.get(flight::String("size")); if (nullish_coalesce_left.has_value()) return nullish_coalesce_left.value(); return 0.0; }()) : -1.0);
}
