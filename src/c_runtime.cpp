...
flight_cpp_status flight_cpp_string_create_utf8(
  ...
  if (!output || (!bytes && size != 0)) return FLIGHT_CPP_STATUS_INVALID_ARGUMENT;
  *output = nullptr;
  try {
...
flight_cpp_status flight_cpp_string_retain(flight_cpp_string* value) {
...
void flight_cpp_string_release(flight_cpp_string* value) {
  if (!value) return;
