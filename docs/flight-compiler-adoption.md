...
| SDL haptics backend | Implemented dependency-closed SDK record | `Flight::HostSdlSdkHaptics` and Bazel `//:host_sdl_sdk_haptics` populate the exact emitted `HapticsBackend` over the first connected SDL gamepad with rumble support. It dynamically discovers replacements, reports amplitude/intensity capabilities, and implements continuous vibration, impact, notification, selection, prepare, and cancel while shared adapter copies retain one owned gamepad reference. SDL has no timed multi-step rumble primitive, so pattern and waveform support are explicitly absent rather than approximated. |
