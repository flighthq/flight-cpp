...
| --- | --- | --- |
| `Required<Row>` row marker | Implemented runtime ABI, awaiting compiler election | `flight::RowRequired<Row>` is the dual of `RowPartial`: it names every member of the row as present. The subject keeps its own storage, so a member it declared optional stays an `std::optional` in the object; what the row changes is that reading yields the value rather than the optional, that a row holding nothing throws instead of returning an empty optional to invoke, and that a write lands back inside the subject's optional. It overrides an inner `RowPartial` rather than inheriting from it, and composes with `RowReadonly` both ways. The spelling for the clipboard case is `StructuralRef<RowRequired<RowOf<Ref<HostClipboardChangeProvider>>>>`. `tests/structural_row_test.cpp` covers it against the committed generated member table rather than a stand-in. Fixed alongside: `named_value` throws on a storage-type mismatch, which made the second step of the existing partial lookup unreachable; readers that accept more than one storage shape now use a non-throwing `named_value_if`. |
| Mutually recursive `Ref<T>` | Implemented runtime ABI | `Ref<T>` answers the same way whether or not `T` is complete, so two records that name each other -- `Node<T>` holding `Ref<NodeRuntime<T>>` while `NodeRuntime<T>` holds a callable over `Ref<Node<T>>` -- are representable. The completeness question is asked through overload resolution instead of a class template specialization keyed on the type, because a specialization cannot be used while it is still being selected and the recursive query re-entered exactly that. A type that names itself through `Ref` must be marked `ReferenceEnabled`, and `make_ref` refuses one where the marker and the answer disagree rather than letting a spelling mean two types. Static assertions pin the answer for records, forward-declared records, value shapes, `void`, and already-reference-backed representations, and a live fixture exercises the recursive pair; the full emitted corpus recompiles unchanged at 1,131 of 1,161 headers. |
