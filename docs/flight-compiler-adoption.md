...
| `ArrayBufferLike` common backing | Prepared runtime ABI | `ArrayBufferLike` now carries ordinary, shared, or host-owned storage with stable identity, lifetime, byte length, mutability, and concurrency policy. Typed arrays and `DataView` construct zero-copy views over it; native tests cover shared and external lifetimes plus read-only rejection. Compiler adoption still needs checker-proven expansion of `ArrayBufferTypes`, compatible bindings for every alternative, and an oracle for the selected closed union. Full concurrent JavaScript `SharedArrayBuffer` operations are not claimed. |
