...
`GPUSamplerDescriptor` values used by Flight's current renderer literals. Optional fields preserve presence for the
provider adapter. `wgpuRenderState.ts` advances from its last ambient refusal to the compiler's
`dual-sentinel optional chaining requires presence projection lowering` boundary; seven other modules advance to
their actual union-evidence, optional-callable, anonymous-property, or `Record`-spread compiler failures. Broader
