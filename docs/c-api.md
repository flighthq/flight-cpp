...
Non-null scalar output slots are initialized to zero before input validation or UTF-8 conversion. Together with the null initialization of handle output slots, this makes every failure path deterministic and prevents callers from observing stale output values. The required byte count replaces zero on `FLIGHT_CPP_STATUS_BUFFER_TOO_SMALL`.
