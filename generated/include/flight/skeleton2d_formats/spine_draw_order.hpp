...
inline std::optional<flight::Array<double>> resolve_spine_draw_ordering(flight::SequenceView<flight::StructuralRef<flight::RowReadonly<flight::RowOf<flight::Ref<offset_slot_index_78b23845efacf0f9>>>>> moves, double slot_count) {
  ...
  for (auto move : moves) {
    ...
    if (((!flight::to_boolean(flight::is_integer(slot_index)) || (slot_index < 0.0)) || (slot_index >= slot_count))) {
      return std::nullopt;
    }
    const double destination = (slot_index + flight::row_get<flight::RowKey<"offset">>(move));
    if (((!flight::to_boolean(flight::is_integer(destination)) || (destination < 0.0)) || (destination >= slot_count))) {
      return std::nullopt;
    }
