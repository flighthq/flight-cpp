// Generated by @flighthq/tool-compiler from @flighthq/geometry/packages/geometry/src/capsule.ts#Capsule at 903f3289590358eacc20698525e6982da2ab0e9d. Do not edit.
#pragma once
#include <cmath>
#include <cstdint>
...
inline flight::Array<double> axis_perpendicular(double abx, double aby, double abz, double ab_len2) {
  ...
  const double abs_x = std::abs(abx);
  const double abs_y = std::abs(aby);
  const double abs_z = std::abs(abz);
  ...
  const double length = std::sqrt((((px * px) + (py * py)) + (pz * pz)));
...
inline void get_closest_point_on_capsule(flight::types::Vector3Like out, flight::StructuralRef<flight::RowReadonly<flight::RowOf<flight::types::CapsuleLike>>> capsule, flight::StructuralRef<flight::RowReadonly<flight::RowOf<flight::types::Vector3Like>>> point) {
  ...
  if ((ab_len2 < 1e-20)) {
    ...
    const double t = flight::minimum(flight::maximum((((((px - ax) * abx) + ((py - ay) * aby)) + ((pz - az) * abz)) / ab_len2), 0.0), 1.0);
    (closest_x = (ax + (t * abx)));
    (closest_y = (ay + (t * aby)));
    (closest_z = (az + (t * abz)));
  }
...
inline double segment_to_aabb_distance_sq(double ax, double ay, double az, double bx, double by, double bz, double min_x, double min_y, double min_z, double max_x, double max_y, double max_z) {
  ...
  if ((len2 < 1e-20)) {
    const double ex = flight::maximum((min_x - ax), 0.0, (ax - max_x));
    const double ey = flight::maximum((min_y - ay), 0.0, (ay - max_y));
    const double ez = flight::maximum((min_z - az), 0.0, (az - max_z));
    ...
    while ((i < count)) {
      {
        ...
        const double ex_2 = flight::maximum((min_x - px), 0.0, (px - max_x));
        const double ey_2 = flight::maximum((min_y - py), 0.0, (py - max_y));
        const double ez_2 = flight::maximum((min_z - pz), 0.0, (pz - max_z));
  ...
  const double t_center = flight::minimum(flight::maximum((((((cx - ax) * dx) + ((cy - ay) * dy)) + ((cz - az) * dz)) / len2), 0.0), 1.0);
  {
    const double px_2 = (ax + (t_center * dx));
    const double py_2 = (ay + (t_center * dy));
    const double pz_2 = (az + (t_center * dz));
    const double ex_3 = flight::maximum((min_x - px_2), 0.0, (px_2 - max_x));
    const double ey_3 = flight::maximum((min_y - py_2), 0.0, (py_2 - max_y));
    const double ez_3 = flight::maximum((min_z - pz_2), 0.0, (pz_2 - max_z));
