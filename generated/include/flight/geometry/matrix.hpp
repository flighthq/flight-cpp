// Generated by @flighthq/tool-compiler from @flighthq/geometry/packages/geometry/src/matrix.ts#Matrix at 7e2fc7df5378313ceaf21ba8d6ef39cc18a6f4ad. Do not edit.
#pragma once
#include <cmath>
#include <cstdint>
...
inline void copy_matrix_column_to_vector3(flight::types::Vector3Like out, double column, flight::StructuralRef<flight::RowReadonly<flight::RowOf<flight::types::MatrixLike>>> source) {
  ...
  const double a = flight::row_get<flight::RowKey<"a">>(object_pattern_value);
  const double b = flight::row_get<flight::RowKey<"b">>(object_pattern_value);
  const double c = flight::row_get<flight::RowKey<"c">>(object_pattern_value);
  const double d = flight::row_get<flight::RowKey<"d">>(object_pattern_value);
  const double tx = flight::row_get<flight::RowKey<"tx">>(object_pattern_value);
  const double ty = flight::row_get<flight::RowKey<"ty">>(object_pattern_value);
  {
...
inline void matrix_transform_bounds(flight::types::RectangleLike out, flight::StructuralRef<flight::RowReadonly<flight::RowOf<flight::types::MatrixLike>>> source, double ax, double ay, double bx, double by) {
  ...
  const double c = flight::row_get<flight::RowKey<"c">>(object_pattern_value);
  const double d = flight::row_get<flight::RowKey<"d">>(object_pattern_value);
  const double source_tx = flight::row_get<flight::RowKey<"tx">>(object_pattern_value);
  const double source_ty = flight::row_get<flight::RowKey<"ty">>(object_pattern_value);
  if (((ax == bx) && (ay == by))) {
    (out->x = source_tx);
    (out->y = source_ty);
    (out->width = 0.0);
...
inline void set_gradient_transform_matrix(flight::types::MatrixLike out, double width, double height, std::optional<double> rotation = std::nullopt, std::optional<double> tx = std::nullopt, std::optional<double> ty = std::nullopt) {
  ...
  if ((rotation.value() != 0.0)) {
    const double cos = std::cos(rotation.value());
    const double sin = std::sin(rotation.value());
    (out->b = (sin * out->d));
    (out->c = (-sin * out->a));
    (out->a *= cos);
    (out->d *= cos);
