...
function memberTable(keys) {
  ...
  // Nothing about the widening proof appears here. The runtime declares a conservative-false
  // primary for it, so a table that adds no specialization simply proves no widening -- which is
  // the right answer for this fixture and is exactly the property that lets a table predating that
  // contract still compile.
